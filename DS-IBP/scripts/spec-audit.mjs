/* ============================================================
   ...
   Проходы (см. «Ревизия ДС: правило объявлено — кода нет. Отчёт»):
     ...
     8. Манифест specs/_index.md против каталога компонентов в AGENTS.md ДС
        (§6), в обе стороны. Каталог — копия манифеста в контексте каждого
        агента; сторож реестров линтера её не читает. Нет файла — строка
        ПРОПУЩЕН.
...
/* ---------- Проход 8: манифест против каталога в правилах агента ----------
   Каталог компонентов в `AGENTS.md` ДС (§6) — ещё одна копия манифеста
   `specs/_index.md`, и она лежит в контексте КАЖДОГО агента. До 21.09.2026
   каталог жил в правилах харнеса (`.opencode/rules/ds-rules.md`), и аудит ДС
   читал файл другой части репозитория: новый компонент требовал правки в
   харнесе. Теперь сверка внутри ДС (реструктуризация, шаг Ш3).
...
let catalogBroken = false;
{
  ...
  if (rules === null) {
    say('ПРОПУЩЕН: нет AGENTS.md ДС — сверять каталог не с чем');
  } else {

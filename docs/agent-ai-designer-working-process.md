...
| Правила ДС | `DS-IBP/AGENTS.md` | **всегда, во всех агентах** | карта ДС, запреты, каркас экрана, каталог компонентов |
...
```bash
find .agents .opencode -name node_modules -prune -o -type f -print | sort
...
| Сквозные принципы: чтение ДС, запреты, не выдумывать неизвестное, вердикт строкой, граница статики, экономия контекста | `process.md` §3, §4, §7, §8, §9, §13; границы записи — `AGENTS.md` и `ai-designer.md` («Границы») |

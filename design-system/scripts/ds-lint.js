...
async function globalChecks(P, out) {
  ...
  /* B11 — компонент, объявивший display, но не объявивший `[hidden]`.
     ...
     не ищет виновных, а закрывает саму возможность (доктрина process.md §9 «что нельзя
...
async function pageChecks(p, P, opts, out) {
  ...
  for (const r of refs) {
    ...
    /* Ссылка, ведущая ВНЕ дерева ДС, этим индексом не проверяется: `P.files`
       строится обходом только внутри design-system (`tree()`). У экрана из
       `Projects/**` ссылка `../../design-system/ds.css` корректна, но индексом не

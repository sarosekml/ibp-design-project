/* =========================================================================
   ...
   детей слоя разрешается в корневом контексте (menu/popover/dropdown 40), а
   `.modal-scrim` несёт 1000 — элемент рисуется ПОД подложкой, а над самой
   модалкой ещё и под её непрозрачным фоном. Тултип (с Tooltip 2.010 —
   максимальный --tip-z) рисовался бы поверх, но вне модалки попадал бы под
   её inert — правило то же. Вторым слоем
   ...
   и chart__tip — максимальный --tip-z, popover/menu/dropdown 40) разрешается в КОРНЕВОМ контексте

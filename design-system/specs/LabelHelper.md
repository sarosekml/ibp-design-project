...
- Не самостоятельные компоненты — нет своего контейнера, не выводятся на экран без родителя. Следствие: в составе экрана и локального компонента (списки «из чего собрано», поле `ds` паспорта) отдельной единицей не перечисляются — их приносит родительское поле (1.007).
...
- **Использование · Правило зависимости** — Label и Helper — не самостоятельные компоненты дизайн-системы. У них нет собственного узнаваемого контейнера: вне контекста родителя их невозможно опознать на экране. Публикуются только как часть композиции Checkbox, Radiobutton, Switch или полей ввода. В составе экрана и локального компонента они отдельной единицей не перечисляются — их приносит с собой родительское поле.

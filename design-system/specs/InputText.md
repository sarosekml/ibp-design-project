...
- **Значение, поставленное кодом, синхронизируется вызовом.** Рантайм считает крестик по значению, а узнаёт о значении из событий `input`/`change` на контроле и из наблюдателя за чипами. Присваивание `input.value = …` не порождает ни того, ни другого: поле выглядит заполненным, а рантайм продолжает считать его пустым и крестика не рисует. Поэтому тот, кто заполняет форму данными, в конце зовёт `DSInput.syncAll(область)` (одно поле — `DSInput.sync(inp)`). Выбор из выпадающего списка звать не нужно: с DropdownList 1.014 `ds-dropdownlist.js` синхронизирует поле сам, сразу после `onSelect`.
...
- «Значение в поле есть, а крестика нет» → либо на странице не подключён `scripts/ds-input.js` (экраны получают его через `scripts/ds.js`, страницы документации подключают поштучно), либо значение поставлено присваиванием `.value` без последующего `DSInput.sync`/`syncAll` — присваивание событий не шлёт, и рантайм считает поле пустым

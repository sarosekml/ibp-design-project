...
/* ============================================================
   ...
   модуля живёт в своей папке (`components/<Имя>/<Имя>.html` приложения).
   Рантайм-инклуд `design-system/scripts/ds-include.js` здесь не годится: он тянет
   ...
   Запуск:
     node apps/postrade/drafts/tools/assemble.mjs                 — собрать все источники направления
     node apps/postrade/drafts/tools/assemble.mjs <источник> […]   — собрать указанные
...
/* Корень приложения: ассемблер лежит в его `tools/`, источники — в `pages/`,
   фрагменты — в `components/` (форма приложения, реструктуризация, Ш7). */
...
function build(inputPath) {
  ...
  console.log('assemble: ' + path.relative(APP, inputPath).split(path.sep).join('/') +
              ' — фрагментов ' + inlined.length + ', CSS компонентов ' + css.length);
  inlined.forEach(function (f) { console.log('  – ' + f); });
  console.log('assemble: готово → ' + path.relative(APP, outputPath).split(path.sep).join('/'));
}

/* Источники направления: любой .html внутри apps/postrade/drafts/ с меткой <ds-include>.
...
if (!targets.length) fail('в ' + APP + ' нет источников с метками <ds-include>');

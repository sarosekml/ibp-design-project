...
file: apps/postrade/drafts/pages/MainPage.html
...
`Portfolio.html` (страница портфеля лежит рядом, в той же `pages/`
приложения). Пункты без собранной
...
- Клик по обычному тайлу — переход на страницу раздела (вся плитка — ссылка).
  ...
  (`PAGE_HREFS['current-portfolio']` → `Portfolio.html`);
